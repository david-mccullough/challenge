...
exports.create = async(body, res) => {
    try {
        ...
        if (!body.rating || body.rating < 0 || body.rating > 5 || !isInt(body.rating)) {
            res.status(400).send({ message: "Rating must an integer be between 1 and 5." });
            return;
        }
...
exports.findAll = async(body, res) => {
    try {
        let cursor = await reviews.find().sort({ date: -1 }).limit(body.limit || 25).toArray();
